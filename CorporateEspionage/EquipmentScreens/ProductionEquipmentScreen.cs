/********************************************
 * Production Equipment Screen Class        *
 * by Mark McCarthy                         *
 *                                          *
...
{
    class ProductionEquipmentScreen : ActiveMenuScreen
    {
        ...
        static string[] equipment = { "Tools", "Assembly Robot", "Fabricator" };
        ...
        public ProductionEquipmentScreen(Player player, Vector2 menuPos, Vector2 titlePos)
            : base("Human Resources", player, menuPos, titlePos)
        {
            ...
            Vector2 pos = anchor;
            basic = new Sprite(player.ProductionSegment.TextureBasic, pos, new Point(32, 48), new Point(0, 0), new Point(1, 1), new Vector2(0, 1), 2f);
            pos.X += 150;
            intermediate = new Sprite(player.ProductionSegment.TextureIntermediate, pos, new Point(32, 48), new Point(0, 0), new Point(1, 1), new Vector2(0, 1), 2f);
            pos.X += 150;
            advanced = new Sprite(player.ProductionSegment.TextureAdvanced, pos, new Point(32, 48), new Point(0, 0), new Point(1, 1), new Vector2(0, 1), 2f);
        }
        ...
        void ConfirmMenuEntrySelected(object sender, PlayerIndexEventArgs e)
        {
            ...
            if (action == Action.Sell)
            {
                if (equipmentIndex == 0)        //can't sell if there are none to sell
                {
                    if (seg.Tools < 1)
                        fail = true;
                    else
                    {
                        seg.Tools--;
                        player.Money += seg.CostTools / 2;      //sell at half purchase value
            ...
            else
            {
                if (equipmentIndex == 0)
                {
                    if (seg.CostTools > player.Money)       //can't buy if insufficient funds
                        fail = true;
                    else
                    {
