...
{
    class ProductionHRScreen : ActiveMenuScreen
    {
        ...
        public ProductionHRScreen(Player player, Vector2 menuPos, Vector2 titlePos)
            : base("Human Resources", player, menuPos, titlePos)
        {
            ...
            Vector2 pos = anchor;
            rookie = new Sprite(player.ProductionSegment.TextureRookie, pos, new Point(16, 24), new Point(0, 0), new Point(2, 4), new Vector2(0, 1), 2f);
            pos.X += 100;
            veteran = new Sprite(player.ProductionSegment.TextureVeteran, pos, new Point(16, 24), new Point(0, 0), new Point(2, 4), new Vector2(0, 1), 2f);
            pos.X += 100;
            expert = new Sprite(player.ProductionSegment.TextureExpert, pos, new Point(16, 24), new Point(0, 0), new Point(2, 4), new Vector2(0, 1), 2f);
        }
        ...
        void ConfirmMenuEntrySelected(object sender, PlayerIndexEventArgs e)
        {
            ...
            if (action == Action.Fire)
            {
                ...
                {
                    if (seg.Rookies < 1)                //can't fire a nonexistent employee
                        fail = true;
                    else
                    {
                        seg.Rookies--;
            ...
            else
            {
                if (employee == Employee.Rookie)
                {
                    if (seg.CostRookie > player.Money)      //can't hire an employee if insufficient funds
                        fail = true;
                    else
                    {
